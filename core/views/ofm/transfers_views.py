...
class TransfersOverviewTableView(CsrfExemptMixin, JsonRequestResponseMixin, View):
    ...
    def get(self, request):
        ...
        table_json['medians'] = self._get_medians(group1, prices, group2)

        return self.render_json_response(table_json)

    @staticmethod
    def _get_medians(group1, prices, group2):
        medians = []
        for group2_item in group2:
            row = []
            for group1_item in group1:
                try:
                    formatted_median = prices.get_group((group1_item, group2_item)).median()
                    row.append(float(formatted_median))
                except KeyError:
                    row.append('-')
            medians.append(row)
        return medians
