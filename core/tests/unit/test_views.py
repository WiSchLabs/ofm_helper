...
class LoginTestCase(TestCase):
    ...
    def test_login_with_wrong_password(self):
        response = self.client.post('/login/', {'username': 'temporary', 'password': 'incorrect'})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, '/login')
        self.assertFalse(response.wsgi_request.user.is_authenticated())
    ...
    def test_login_with_inactive_account(self):
        ...
        response = self.client.post('/login/', {'username': 'disabled', 'password': 'disabled'})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, '/login')
        self.assertFalse(response.wsgi_request.user.is_authenticated())
...
class RegistrationTestCase(TestCase):
    ...
    def test_register_new_account(self):
        response = self.client.post('/register/', {'username': 'new', 'password': '1234', 'password2': '1234',
    ...
    def test_register_with_existing_username(self):
        response = self.client.post('/register/', {'username': 'temporary', 'password': '1234', 'password2': '1234',
                                         'email': 'new@ofmhelper.com',
                                         'ofm_username': 'abc', 'ofm_password': 'def', 'ofm_password2': 'def'})
        self.assertEqual(response.status_code, 302)
    ...
    def test_register_with_unmatching_passwords(self):
        response = self.client.post('/register/', {'username': 'new', 'password': '1234', 'password2': '12345',
                                         'email': 'new@ofmhelper.com',
                                         'ofm_username': 'abc', 'ofm_password': 'def', 'ofm_password2': 'def'})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, '/register')
